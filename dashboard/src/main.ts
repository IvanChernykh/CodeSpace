...
import type { TabId, WorkspaceRegistrySnapshot } from "./types.js";
...
function bootstrap(): void {
  ...
  const statusDot = qs<HTMLElement>("#statusDot");
  const statusText = qs<HTMLElement>("#statusText");
  const updateButton = qs<HTMLButtonElement>("#updateIndexBtn");
  const doctorButton = qs<HTMLButtonElement>("#doctorBtn");
  const paletteTrigger = qs<HTMLButtonElement>("#paletteTrigger");
  const workspaceSelect = qs<HTMLSelectElement>("#workspaceSelect");

  const graphSvg = qs<SVGSVGElement>("#graphSvg");
  const graphEmptyState = qs<HTMLElement>("#graphEmptyState");
  const sidebarRoot = qs<HTMLElement>("#sidebar");
  const inspectorRoot = qs<HTMLElement>("#inspector");
  const contextRoot = qs<HTMLElement>("#panel-context");
  const impactRoot = qs<HTMLElement>("#panel-impact");
  const historyRoot = qs<HTMLElement>("#panel-history");
  const workspacesRoot = qs<HTMLElement>("#panel-workspaces");
  const paletteOverlay = qs<HTMLElement>("#commandPalette");

  function selectSymbol(id: number | null): void {
  ...
  function refreshWorkspaceSelect(snapshot: WorkspaceRegistrySnapshot): void {
    store.set({ workspaces: snapshot });
    workspaceSelect.replaceChildren(
      ...snapshot.workspaces.map((ws) =>
        el("option", { value: ws.id, selected: ws.active }, [ws.name]),
      ),
    );
  ...
  function switchTab(tab: TabId): void {
    ...
    for (const nav of qsa<HTMLElement>("[data-tab]")) {
      nav.classList.toggle("is-active", nav.dataset.tab === tab);
    }
    for (const panel of qsa<HTMLElement>("[data-panel]")) {
  ...
  for (const nav of qsa<HTMLElement>("[data-tab]")) {
    nav.addEventListener("click", () => {
      const tab = nav.dataset.tab as TabId;
      switchTab(tab);
      if (tab === "workspaces") void workspacesPanel.load();
    });
  }
  ...
  doctorButton.addEventListener("click", async () => {
    ...
    try {
      ...
      for (const message of result.messages) toast.info(message);
      if (result.messages.length === 0) toast.success("Everything looks healthy");
    } catch (error) {
  ...
  for (const checkbox of qsa<HTMLInputElement>("[data-edge-filter]")) {
    checkbox.addEventListener("change", () => {
      const kind = checkbox.dataset.edgeFilter;
      if (kind) graphView.setEdgeFilter(kind as never, checkbox.checked);
    });
  }
  ...
  document.addEventListener("keydown", (ev) => {
    ...
    if (ev.key === "/") {
      ev.preventDefault();
      switchTab("graph");
      qs<HTMLInputElement>("[data-sidebar-filter]", sidebarRoot).focus();
      return;
    }
    const index = TABS.indexOf(ev.key as TabId);
  ...
  liveEvents.onEvent((event) => {
    switch (event.type) {
      ...
      case "workspace.selected":
        ...
        break;
      case "decision.added":
        if (store.get().activeTab === "history") void historyPanel.run();
        break;
      default:
        break;
    }
  ...
  window.addEventListener("resize", () => {
    const graph = store.get().graph;
    if (graph) graphView.setGraph(graph);
  });
