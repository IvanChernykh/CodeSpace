...
import type { EdgeRecord, FileRecord, GraphSnapshot, SymbolRecord } from "./types.js";
import { copyToClipboard, el, escapeHtml, qs } from "./dom.js";
import type { ToastManager } from "./toast.js";

export interface InspectorOptions {
  api: ApiClient;
  toast: ToastManager;
  onFocusSymbol: (id: number) => void;
}
...
export class Inspector {
  ...
  constructor(root: HTMLElement, options: InspectorOptions) {
    this.root = root;
  ...
  render(symbol: SymbolRecord, graph: GraphSnapshot): void {
    ...
    this.root.replaceChildren(
      ...
      this.field("Location", file ? `${file.path}:${symbol.line_start}-${symbol.line_end}` : "unknown"),
      this.field("Complexity", String(symbol.complexity)),
      el("div", { class: "field" }, [
      ...
      symbol.doc
        ...
        : el("div", {}),
      this.relationshipsSection(graph, outgoing, incoming),
      this.sourceSection(file, symbol),
    );
  }
  ...
  private copyButton(value: string): HTMLElement {
    const button = el("button", { class: "icon-button", type: "button", title: "Copy qualified name" }, ["\u2398"]);
    button.addEventListener("click", async () => {
      const ok = await copyToClipboard(value);
      if (ok) this.options.toast.success("Copied qualified name");
      else this.options.toast.error("Clipboard is unavailable");
    });
    return button;
  }

  private relationshipsSection(
    graph: GraphSnapshot,
    outgoing: EdgeRecord[],
    incoming: EdgeRecord[],
  ): HTMLElement {
  ...
  private sourceSection(file: FileRecord | undefined, symbol: SymbolRecord): HTMLElement {
...
export function selectedSymbolBanner(root: HTMLElement, text: string): void {
