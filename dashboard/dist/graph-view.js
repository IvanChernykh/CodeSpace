...
export class GraphView {
    constructor(svg, options) {
        ...
        this.panStart = { x: 0, y: 0, tx: 0, ty: 0 };
        this.rafHandle = null;
        this.settleTicks = 0;
        this.handleWheel = (ev) => {
            ev.preventDefault();
    ...
    setGraph(graph) {
        ...
        const ranked = [...graph.symbols].sort((a, b) => (degree.get(b.id) ?? 0) - (degree.get(a.id) ?? 0));
        ...
        this.edges = [];
        for (const edge of graph.edges) {
            if (!chosenIds.has(edge.from) || !chosenIds.has(edge.to))
                continue;
