...
**Local-first semantic code graph and compact context engine for AI coding agents.**

`cse` indexes a repository, extracts symbols and relationships, ranks task-relevant code, redacts likely secrets, analyzes Git change impact, preserves engineering decisions, and exposes the result through CLI, MCP, REST, and a Rust library.
...
Version 1.0.0 is a complete, dependency-free reference implementation of the requested product surface. It is intentionally conservative:
...
| Area | 1.0 implementation |
|---|---|
| Repository index | Incremental hashing, ignored directories, size limits, symlink protection |
| Languages | Rust, Python, JS/TS, Go, Java/Kotlin, C/C++, C#, Swift, PHP, Ruby, shell, and structural fallback formats |
| Graph | Files, symbols, containment, imports, calls, confidence scores |
| Context | Lexical + graph ranking, bounded source excerpts, comment/whitespace compaction, token budget |
| Security | Path confinement, local-only defaults, secret redaction, binary/large-file exclusion |
...
| Interfaces | CLI, MCP stdio, loopback REST, Rust library |
| Export | JSON, Graphviz DOT, standalone interactive HTML |
| Operations | Watch mode, lock recovery, doctor, stats, benchmark, shell |
