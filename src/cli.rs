    ...
    I: IntoIterator<Item = String>,
{
    ...
    match command {
        ...
        "workspace" | "ws" => command_workspace(&parsed),
        "skills" => command_skills(&parsed),
...
fn command_find(args: &ParsedArgs) -> Result<i32> {
    ...
    let limit = args
        ...
        .unwrap_or(20)
        .clamp(1, 500);
    let hits = crate::search::find_symbols(&graph, &query, kind, limit);
...
fn command_impact(args: &ParsedArgs) -> Result<i32> {
    ...
    let depth = args
        ...
        .unwrap_or(3)
        .clamp(1, 10);
    let report = crate::impact::analyze(&root, &graph, from, to, depth)?;
...
fn command_history(args: &ParsedArgs) -> Result<i32> {
    ...
    let limit = args
        ...
        .unwrap_or(10)
        .clamp(1, 1_000);
    let decisions = crate::memory::history(&graph, target, limit);
    let rendered = match args.value("format").map_or("plain", String::as_str) {
...
fn command_remember(args: &ParsedArgs) -> Result<i32> {
    ...
    let id = crate::memory::remember(&mut graph, input);
...
fn command_serve(args: &ParsedArgs) -> Result<i32> {
    ...
    if args.flag("dashboard") || args.flag("server") {
        let port = args
            .value("port")
            .and_then(|value| value.parse::<u16>().ok())
            .unwrap_or(8080);
        let config = crate::server::ServerConfig::new(port);
        crate::server::serve(&root, config)?;
        return Ok(0);
    }
...
fn command_stats(args: &ParsedArgs) -> Result<i32> {
    ...
    if args.value("format").is_some_and(|format| format == "json") || args.flag("json") {
        let language_json = languages
            ...
            .join(",");
        println!(
            "{{\"project_root\":\"{}\",\"files\":{},\"symbols\":{},\"edges\":{},\"decisions\":{},\"index_revision\":{},\"index_bytes\":{},\"updated_unix_ms\":{},\"languages\":{{{}}}}}",
        ...
        println!("Index revision: {}", graph.index_revision);
...
fn command_workspace(args: &ParsedArgs) -> Result<i32> {
    let sub = args.positionals.first().map_or("list", String::as_str);
    match sub {
        "list" | "ls" => {
        ...
        other => Err(Error::InvalidArgument(format!(
            "unknown workspace subcommand `{other}`; try list, register, remove, select"
        ))),
    }
}

fn command_skills(args: &ParsedArgs) -> Result<i32> {
    let sub = args.positionals.first().map_or("list", String::as_str);
    let mut registry = crate::skills::load_skill_registry();
    match sub {
        "list" | "ls" => {
            if args.value("format").is_some_and(|f| f == "json") {
                println!("{}", registry.to_json());
        ...
        other => Err(Error::InvalidArgument(format!(
            "unknown skills subcommand `{other}`; try list, enable, disable, uninstall"
        ))),
    }
}

fn command_settings(args: &ParsedArgs) -> Result<i32> {
    let sub = args.positionals.first().map_or("list", String::as_str);
    match sub {
        "list" | "get" => {
            let chain = crate::settings::SettingsChain::new();
            let key = args.positionals.get(1);
            if let Some(key) = key {
                match chain.get(key) {
        ...
        other => Err(Error::InvalidArgument(format!(
            "unknown settings subcommand `{other}`; try list, set"
        ))),
    }
}

fn command_read(args: &ParsedArgs) -> Result<i32> {
    let root = root_from_args(args)?;
    let file = args.positionals.first().map_or("", String::as_str);
    if file.is_empty() {
    ...
    let redacted = crate::secret::redact_secrets(&output);
    println!(
        "file: {relative}\nredactions: {}\n{}",
...
fn command_graph(args: &ParsedArgs) -> Result<i32> {
    let root = root_from_args(args)?;
    let graph = storage::load(&root)?;
    let format = args.value("format").map_or("json", String::as_str);
    let rendered = match format {
...
WORKSPACE COMMANDS
  cse workspace list
  cse workspace register <path> [--name <name>]
  cse workspace remove <id>
  cse workspace select <id>

SKILLS COMMANDS
  cse skills list
  cse skills enable <id>
  cse skills disable <id>
  cse skills uninstall <id>

SETTINGS COMMANDS
  cse settings list [key]
  cse settings set <key> <value> [--scope global|workspace]
