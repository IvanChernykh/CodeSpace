...
pub fn to_json(graph: &GraphIndex) -> String {
    let mut output = format!(
        "{{\"schema_version\":{},\"project_root\":\"{}\",\"created_unix_ms\":{},\"updated_unix_ms\":{},\"index_revision\":{},\"files\":[",
    ...
    for (index, edge) in graph.edges.iter().enumerate() {
        ...
        output.push_str(&format!(
            "{{\"from\":{},\"to\":{},\"kind\":\"{}\",\"confidence_milli\":{},\"precision\":\"{}\",\"evidence\":\"{}\"}}",
            edge.from,
            edge.to,
            edge.kind.as_str(),
            edge.confidence_milli,
            edge.precision.as_str(),
            json_escape(&edge.evidence)
        ));
    }
    output.push_str("]}");
    output
